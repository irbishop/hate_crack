...
class RuleGen:
        ...
        for hashcat_rule in sorted(hashcat_rules, key=lambda hashcat_rule: len(hashcat_rule)):
            ...
            if not self.more_rules:
                if rule_length < best_found_rule_length:
                    ...
                    if self.verbose:
                        print(f"[-] {suggestion} => (best rule length exceeded: {rule_length} ({best_found_rule_length})) => {password}")
                    break
    ...
    def generate_simple_hashcat_rules(self, word, rules, password):
        ...
        if self.debug:
            print("[*] Simple Processing {word} => {password}")
        ...
        for (op, p, w) in rules:
            ...
            if self.debug:
                print("\t[*] Simple Processing Started: {word_rules} - {' '.join(hashcat_rules)}")
        ...
        if self.debug:
            print("\t[*] Simple Processing Ended: {word_rules} => {' '.join(hashcat_rules)} => {password}")
        ...
        if word_rules == password:
            ...
            if self.debug:
                print(f"[!] Simple Processing FAILED: {word} => {' '.join(hashcat_rules)} => {password} ({word_rules})")
            return None
    ...
    def generate_advanced_hashcat_rules(self, word, rules, password):
        ...
        for i, (op, p, w) in enumerate(rules):
            ...
            if self.debug:
                print("\t[*] Advanced Processing Started: {word_rules} - {' '.join(hashcat_rules)}")
            ...
            if op == 'insert':
                ...
                if word_rules[p] == password[p]:
                    if self.debug:
                        print(f"\t[*] Advanced Processing Obsolete Rule: {word_rules}- {' '.join(hashcat_rules)}")
        ...
        if self.debug:
            print(f"\t[*] Advanced Processing Ended: {word_rules} {' '.join(hashcat_rules)}")
        ...
        if word_rules == password:
            ...
            if self.debug:
                print(f"[!] Advanced Processing FAILED: {word} => {' '.join(hashcat_rule)} => {password} ({word_rules})")
            return None
    ...
    def check_reversible_password(self, password):
        ...
        if password.isdigit():
            if self.verbose and not self.quiet:
                print("[!] {password} => (skipping numeric) => {password}")
        ...
        # TODO: Make random word detection more reliable based on word entropy.
            ...
            if self.verbose and not self.quiet:
                print("[!] {password} => (skipping alpha less than 25%%) => {password}")
    ...
    def print_hashcat_rules(self, words, password, rules_queue, words_queue):
        ...
        for word in sorted(words, key=lambda word: len(word["hashcat_rules"][0]) if (len(word["hashcat_rules"]) > 0) else 0):
            ...
            for hashcat_rule in word["hashcat_rules"]:
                ...
                if not self.more_rules:
                    if rule_length < best_found_rule_length:
                        ...
                        if self.verbose:
                            print(f"[-] {word['suggestion']} => (best rule length exceeded: {rule_length} ({best_found_rule_length})) => {password}")
                        break
                if rule_length <= self.max_rule_len:
                    hashcat_rule_str = " ".join(hashcat_rule + word["pre_rule"] or [':'])
                    if self.verbose:
                        print(f"[+] {word['suggestion']} => {hashcat_rule_str} => {password}")
                    rules_queue.put(hashcat_rule_str)

    def password_worker(self, i, passwords_queue, rules_queue, words_queue):
        if self.debug:
            print(f"[*] Password analysis worker [{i}] started.")
        try:
            while True:
            ...
            if self.debug:
                print(f"[*] Password analysis worker [{i}] terminated.")

        if self.debug:
            print(f"[*] Password analysis worker [{i}] stopped.")
    ...
    def analyze_passwords_file(self, passwords_file):
        ...
        for i in range(multiprocessing.cpu_count()):
            ...
            multiprocessing.Process(target=self.rule_worker, args=(rules_queue, f"{self.basename}.rule")).start()
        multiprocessing.Process(target=self.word_worker, args=(words_queue, f"{self.basename}.word")).start()
                ...
                for password in f:
                    ...
                    if len(password) > 0:
                        ...
                        if not self.quiet and password_count != 0 and password_count % 5000 == 0:
                            ...
                            print(f"[*] Processed {password_count} passwords in {segment_start-analysis_start:.2f} seconds at the rate of {5000/segment_time:.2f} p/sec")
        ...
        print(f"[*] Finished processing {password_count} passwords in {analysis_time:.2f} seconds at the rate of {(float(password_count)/analysis_time):.2f} p/sec")
        print(f"[*] Generating statistics for [{self.basename}] rules and words.")
        print(f"[-] Skipped {self.numeric_stats_total} all numeric passwords ({float(self.numeric_stats_total)*100/float(password_count):0.2f}%%)")
        print(
